...
function VermisInstitutos() {
    ...
    const fetchUserLabs = async (userId, token) => {
        try {
            const response = await axios.get(`${API_BASE_URL}vermisinsts/${userId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Accept': 'application/json',
                },
            ...
            >
                ...
                <TableContainer >
                    <Table>
                        <TableHead>
                            <TableRow>
                                <TableCell sx={{ textAlign: 'center', fontWeight: 'bold', fontSize: '110%', width: '37%' }}>Instituto</TableCell>
                        ...
                        <TableBody>
                            {paginatedUsers.map((user, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell sx={{ textAlign: 'center' }}>{user.director}</TableCell>
...
export default VermisInstitutos;
